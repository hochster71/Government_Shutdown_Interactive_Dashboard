...
describe('API Impact Calculator', () => {
  ...
  describe('GET /api/news', () => {
    it('should accept query with pageSize parameter', async () => {
      const response = await request(app)
        .get('/api/news')
        .query({ pageSize: 10 });

      // Returns 200 or 500 (500 if NewsAPI key not configured)
      expect([200, 500]).toContain(response.status);
    });

    it('should accept query with sortBy parameter', async () => {
      const response = await request(app)
        .get('/api/news')
